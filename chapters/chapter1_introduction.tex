...
The most infamous reentrancy incident, the DAO attack of June 2016, resulted in approximately \$60 million in losses and prompted an unprecedented hard fork of the Ethereum blockchain~\cite{the-dao-analysis}. \hlc{The incident catalyzed significant defensive efforts: the Checks-Effects-Interactions coding pattern became standard practice~\cite{checks-effects-interactions}, language-level reentrancy guards were introduced, and numerous static and dynamic analysis tools were developed to detect reentrancy vulnerabilities~\cite{oyente, slither, sereum}. One might expect that such extensive defenses would have mitigated the threat.}
%\karthik{This may be misleading; there was work on defenses, correct? Perhaps say that first and then say one would expect these should have mitigated them.}
Yet nearly a decade later, these attacks continue to occur with alarming regularity. Our analysis documents at least \integer{ReentrancyCount} confirmed reentrancy incidents across EVM-compatible blockchains between 2016 and 2024, with a frequency exceeding 1.5 attacks per month since mid-2021.
...
The continued prevalence of reentrancy attacks despite significant academic attention
% \karthik{You need to first summarize this.} Summarized in the previous section
indicates fundamental gaps in both our understanding and our defensive capabilities. Three problems motivate this research.
...
\end{itemize}
%\karthik{There have been other studies that found this correct - perhaps mention this.}
\hlc{These assumptions were reinforced by early studies and security guidelines~\cite{smart-contract-security-field-guide, rethinking-of-reentrancy}.} However, the attack landscape has evolved considerably since these assumptions were formulated. Preliminary observations suggest that modern attackers employ increasingly sophisticated techniques, exploit diverse entry points, and target complex multi-contract interactions. Without systematic empirical investigation of real-world attacks, the research community operates on potentially outdated assumptions that may not reflect current threats.

\subsection{Detection Tool Limitations}

Existing reentrancy detection tools have demonstrated limited effectiveness against modern attacks. \hlc{Prominent tools such as Oyente~\cite{oyente}, Slither~\cite{slither}, Securify~\cite{securify}, and Sereum~\cite{sereum} were designed primarily to detect classic single-function reentrancy patterns.} Recent benchmark studies reveal troubling performance gaps: Zheng~\etal~\cite{turn-the-rudder} found that state-of-the-art tools suffered false positive rates up to 99.8\% when evaluated on real Ethereum transactions, while failing to detect non-classic reentrancy patterns. Similarly, Ghaleb~\etal~\cite{survey:static-analysis} documented substantial false positive and negative rates across six widely-used static analyzers.
%\karthik{I think you should specifically look at Reentrancy attack detection tools here.}
...
\end{itemize}
\hlc{We describe our own experience with this manual analysis process in detail in Section~\ref{sec:meth-manual}, where we outline the steps involved in analyzing each incident in our dataset.}
%\karthik{Need citations above. Also, do we show an example of manual analysis later in the thesis?}
...
For each incident, two researchers independently analyze the exploit transaction's call trace and token flows, identify the reentrancy pattern and vulnerability, and characterize the attack along multiple dimensions. This analysis employs an \hlc{open coding methodology~\cite{open-coding:strauss-corbin, open-coding:saldana}} to develop a taxonomy of attack strategies, entry points, and scope classifications.
%\karthik{We should cite open-coding papers and ensure our methodology is in line with theirs. }
Cross-validation ensures consistency and reduces bias.
...
% \karthik{This is good, but then we need to update our findings and insights to include both the manual and automated analysis.} % That's exactly how I organized the results chapter
